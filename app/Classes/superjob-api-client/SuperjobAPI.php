...
class SuperjobAPI
{
  ...
  public function customGetVacancies(array $data = array())
  {
    $vacancies = array();
    $vacanciesInfo = $this->vacancies($data);
    $vacanciesTotal = $vacanciesInfo['total'];
    $vacanciesPages = (int)($vacanciesTotal / 10);

    foreach ($vacanciesInfo['objects'] as $vacancy)//Добавляем элементы из первой страницы ( нулевой в массиве )
    {
      $vacancies[] = [
    ...
    if ($vacanciesPages > 0)// Если не одна страница , то пройдемся по всем начиная со 2 страницы ( в массиве 1 )
    {
      for ($i = 1; $i <= $vacanciesPages; $i++) {
        $rawVacancies = $this->vacancies([// Запрашиваем данные в сыром виде
            "town" => $data['town'],
        ...
        foreach ($rawVacancies['objects'] as $vacancy)// Нужные данные вставляем в массив ($rawVacancies['objects'] - массив вакансий)
        {
          $vacancies[] = [
              'name' => $vacancy['profession'],
              'requirement' => $vacancy['candidat'],
